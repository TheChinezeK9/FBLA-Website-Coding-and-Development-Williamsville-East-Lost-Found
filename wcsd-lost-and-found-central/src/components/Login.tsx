...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const SloganBlock = ({ compact = false }: { compact?: boolean }) => (
    <div className="text-center">
      <div className={`mb-2 flex items-center justify-center gap-2 whitespace-nowrap text-white ${compact ? 'text-base' : 'text-lg sm:text-xl'}`}>
        <Flame size={compact ? 20 : 22} color="url(#login-red-orange-gradient)" />
      ...
      <p className={`${compact ? 'mb-2 text-[15px]' : 'mb-3 text-sm'} text-white`}>
        Turning lost into found, because lost shouldn’t stay lost.
      </p>
      <p className={`flex items-center justify-center gap-2 text-white ${compact ? 'text-sm' : 'text-xs'}`}>
        <ShieldCheck size={compact ? 16 : 16} color="url(#login-red-orange-gradient)" />
